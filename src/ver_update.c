...
int get_latest_version()
{
    ...
    ret = download_file("http://sourceforge.net/projects/happy-sudoku/files/", TMP_VER_FILE);
    if (0!=ret) return -1;

    len = get_data_from_file(buf, TMP_VER_FILE, sizeof(buf));
    if (len<=0) return -1;

    p_hdr = strstr(buf, "happy_sudoku_v");
    p_end = strstr(buf, ".zip");
    
    if (p_hdr==NULL || p_end==NULL) return -1;
    p_hdr+=strlen("happy_sudoku_v");
...
DWORD WINAPI  ver_update(LPVOID lpParameter)
{
    ...
    hWnd = CreateWindow ("ver_update", NULL,
